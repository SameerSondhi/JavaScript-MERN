...
function musicRuntime(busDuration, songDurations) {
    let time = busDuration-30;
    let result=[]
    for(let i=0;i<songDurations.length; i++){
        let max=0
        if(songDurations[max]>songDurations[i]){
            max=i
        }
    }
    for(let j=songDurations.length-1;j>0; j--){
        let min=0
        if(songDurations[min]<songDurations[j]){
            min=j
        }
    }
    if(songDurations[max]+songDurations[min] == time){
        result=[max, min]
    }
    return result
}
